...
> AI Security Assessment Platform v1.0.3  
...
1. 解压 `ai-sec-check-v1.0.3-windows-amd64.zip` 到目标目录
...
# 或手动构建
set CGO_ENABLED=0
go build -ldflags "-X ai-sec-check/internal/options.version=v1.0.3 -s -w" -o ai-sec-check.exe ./cmd/cli
```
...
| Request Template | 文本输入 | 请求体模板，支持 `$INPUT`（探针提示词）和 `$KEY`（API密钥）占位符 |
...
set GOOS=linux
set GOARCH=amd64
go build -ldflags "-X ai-sec-check/internal/options.version=v1.0.3 -s -w" -o ai-sec-check-linux-amd64 ./cmd/cli

# macOS arm64
set GOOS=darwin
set GOARCH=arm64
go build -ldflags "-X ai-sec-check/internal/options.version=v1.0.3 -s -w" -o ai-sec-check-darwin-arm64 ./cmd/cli
```
