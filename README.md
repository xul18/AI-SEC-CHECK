...
> AI Security Assessment Platform v1.0.5  
...
| 自定义越狱检测 | model_safety | 自定义HTTP请求配置的越狱检测 | Go 原生 (支持自定义请求模板、响应路径) |
...
- **零依赖部署**：7大插件全部 Go 原生实现，纯静态编译，单文件部署
...
- **插件架构**：7大扫描模块即插即用，支持动态启用/禁用，固定显示顺序
- **多接口**：Web界面 + REST API + CLI命令行
- **配置持久化**：AI配置保存到配置文件，重启后自动恢复
- **自定义越狱检测**：支持自定义HTTP请求模板，适配非标准API接口的越狱检测
...
1. 解压 `ai-sec-check-v1.0.5-windows-amd64.zip` 到目标目录
...
# 或手动构建
set CGO_ENABLED=0
go build -ldflags "-X ai-sec-check/internal/options.version=v1.0.5 -s -w" -o ai-sec-check.exe ./cmd/cli
```
...
| Probe Filter | 下拉选择 | 探针过滤：All Probes (37) / Prompt Injection (21) / DAN / Roleplay / Multimodal 等 |
| Concurrency | 下拉选择 | 并发数：1 / 3 / 5 / 10 |

**Garak Custom插件专属参数**（选择garak_custom插件时自动显示）：

| 参数 | 类型 | 说明 |
|------|------|------|
| Model Name | 文本输入 | 目标模型名称 |
| API Key | 密码输入 | 认证密钥 |
| HTTP Method | 下拉选择 | 请求方法：GET / POST / PUT / DELETE |
| Request Template | 文本输入 | 请求体模板，支持 `$INPUT`（探针提示词）、`$KEY`（API密钥）、`$IMAGE_URL`（图像数据）、`$AUDIO_DATA`（音频数据）、`$FILE_CONTENT`（文件内容）占位符 |
| Response Path | 文本输入 | 响应提取路径（JSONPath语法），如 `$.choices[0].message.content` |
| Probe Filter | 下拉选择 | 探针过滤 |
| Concurrency | 下拉选择 | 并发数 |
...
| multimodal | 5 | image_upload, audio_transcribe, file_upload, multimodal_injection, visual_bypass |
...
curl -X POST http://localhost:8088/api/v1/plugins/scan \
  -H "Content-Type: application/json" \
  -d '{"plugin_name":"garak","target_type":"url","target":"http://localhost:8000/v1","metadata":{"target_name":"deepseek-r1:32b","probes":"multimodal","api_key":"sk-xxx"}}'
```

### 7.3 自定义越狱检测（garak_custom）

**类别**：model_safety | **实现**：Go 原生 | **依赖**：需要可访问的LLM API端点

| 目标类型 | 说明 |
|----------|------|
| url | LLM API端点（如 http://localhost:8000/v1） |

支持自定义HTTP请求配置的越狱检测模块，适配非标准API接口的越狱检测。

**可配置参数**（通过metadata传递）：

| 参数 | Key | 说明 |
|------|-----|------|
| Model Name | target_name | 目标模型名称 |
| API Key | api_key | 认证密钥 |
| HTTP Method | http_method | 请求方法：GET / POST / PUT / DELETE（默认POST） |
| Request Template | body_template | 请求体模板，支持占位符替换 |
| Response Path | response_path | 响应提取路径（JSONPath语法），如 `$.choices[0].message.content` |
| Probe Filter | probes | 探针过滤（按类别或名称） |
| Concurrency | max_concurrency | 并发数（默认3） |
...
| 占位符 | 替换内容 | 使用场景 |
|--------|---------|---------|
| `$INPUT` | 探针的文本prompt | 所有探针 |
| `$KEY` | API密钥 | 所有探针 |
| `$IMAGE_URL` | 示例图片base64数据 | multimodal/image探针 |
| `$AUDIO_DATA` | 音频数据占位符 | multimodal/audio探针 |
| `$FILE_CONTENT` | 文件内容占位符 | multimodal/file探针 |

**示例请求模板**（适用于支持图像输入的多模态模型）：
...
curl -X POST http://localhost:8088/api/v1/plugins/scan \
  -H "Content-Type: application/json" \
  -d '{
    "plugin_name":"garak_custom",
    "target_type":"url",
    "target":"http://localhost:8000/v1",
    "metadata":{
      "target_name":"deepseek-r1:32b",
      "api_key":"sk-xxx",
      "http_method":"POST",
      "body_template":"{\"model\":\"deepseek-r1:32b\",\"messages\":[{\"role\":\"user\",\"content\":\"$INPUT\"}]}",
...
- 智能误报过滤：
  ...
  - HTML登录页面跳转（响应包含 `<html>`、`login`、`form`、`password` 等特征）不判为未授权访问
...
set GOOS=linux
set GOARCH=amd64
go build -ldflags "-X ai-sec-check/internal/options.version=v1.0.5 -s -w" -o ai-sec-check-linux-amd64 ./cmd/cli

# macOS arm64
set GOOS=darwin
set GOARCH=arm64
go build -ldflags "-X ai-sec-check/internal/options.version=v1.0.5 -s -w" -o ai-sec-check-darwin-arm64 ./cmd/cli
```
...
- **改进敏感词检测插件**：
  - 增强.doc文件支持，添加RTF格式解析，能够正确提取Word文档中的文本内容进行敏感词检测
  - 添加`extractRTFFromOLE2()`函数，优先从OLE2文件中提取RTF格式内容进行解析
  - 添加`parseRTF()`函数，支持RTF标签移除、特殊字符转换（`\n`、`\t`、`\r`、`\f`）、Unicode字符解码（`\uXXXX`）
  - 导入`strconv`包用于RTF中Unicode字符的解析
- **改进Garak Custom插件**：
  - 增加多模态（multimodal）探针支持，request template支持自动替换图像、音频和文件内容占位符
  - 新增占位符：`$IMAGE_URL`（图像数据）、`$AUDIO_DATA`（音频数据）、`$FILE_CONTENT`（文件内容）
  - 替换机制参考garak模块，支持在请求体和请求头中进行占位符替换

### v1.0.4

- **改进敏感词检测插件**：
  - 支持更多文件格式：`.doc`、`.wps`、`.xlsx`、`.xls`、`.et`、`.ett`、`.xlsm`、`.xlsb`、`.docm`、`.dotx`、`.dotm`
  - 修复OLE2格式文件解析问题，使用正确的GBK编码转换（基于 `golang.org/x/text` 库）
  - 修复文件内容被当成文件名的问题，从 `metadata["file_name"]` 获取正确文件名
  - 优化文本提取逻辑，支持ASCII和GBK混合编码的OLE2文件
- **修复Garak Custom插件**：
  - 参数传递修复：将 `body_template`、`http_method`、`response_path`、`headers` 等参数放到 `metadata` 中正确传递
  - 修复文件名为空时的处理逻辑
- **优化前端文件上传**：
  - 扩展 `readAsDataURL` 支持范围，包含所有办公文档格式（`.doc`、`.wps`、`.xlsx`、`.xls`、`.et` 等）
  - 确保二进制文件正确转换为base64格式传递到后端

### v1.0.3

- **新增 Garak Custom 插件**：支持自定义HTTP请求配置的越狱检测模块，可配置HTTP方法、请求模板、响应路径等参数
- **改进 Auto Swagger 授权扫描**：增加HTML跳转判断，当返回200时检测响应内容是否包含登录页面特征（如 `<html>`、`login`、`form` 等），避免将跳转到登录页误判为未授权访问
- **优化前端界面**：
  - 修复 scanBtn 按钮垂直对齐问题，添加内联样式 `margin-top:16px`
  - 修复输入框长度不足问题，添加 `width: 100%` 和 `box-sizing: border-box`
  - 固定插件显示顺序，避免随机排序
- **改进智能误报过滤**：增加 `isHTMLRedirect()` 方法，检测HTML响应是否为登录页面跳转，提高授权扫描准确性

### v1.0.0

- 初始版本，包含6大扫描模块：敏感词检测、提示词注入检测、基础设施扫描、MCP安全扫描、API授权扫描、限流熔断验证
